...
*/
Ephemerides.eccentricAnomaly1 = function(M, e) {
    /*
...
** failed to converge.
*/
Ephemerides.eccentricAnomaly2 = function(M, e, E0) {
    /*
    ** iteratively refine the eccentric anomaly from a first estimate
    */
...
*/
Ephemerides.eccentricAnomaly4 = function(M, e) {
    with(Math) {
        return M + degrees(e) * sin(radians(M)) * (1.0 + e *cos(radians(M)));
    }
}
...
*/
Ephemerides.eccentricAnomaly = function(M, e) {

    return Ephemerides.eccentricAnomaly4(M, e);
}
...
Ephemerides.Sun = {
    ...
    position: function(eph, planet) {
        ...
        with (Math) {
            with (eph.Sun) {
    //          eph.Sun.E = Ephemerides.(M, e, M + degrees(e) * sin(radians(M)) * (1.0 + e * cos(radians(M))));
                eph.Sun.E = Ephemerides.eccentricAnomaly(M, e, M);
...
//          eph[planet].E = Ephemerides.eccentricAnomaly(M, e, M + degrees(e) * sin(radians(M)) * (1.0 + e * cos(radians(M))));
            eph[planet].E = Ephemerides.eccentricAnomaly(M, e, M);
...
Ephemerides.Mercury = {
    ...
    mag: function(r, R, FV) { with (Math) return -0.36 + 5*log10(r*R) + 0.027 * FV + 2.2E-13 * pow(FV,6); }
};
...
Ephemerides.Venus = {
    ...
    mag: function(r, R, FV) { with (Math) return -4.34 + 5*log10(r*R) + 0.013 * FV + 4.2E-7  * pow(FV,3) }
};
...
Ephemerides.Mars = {
    ...
    mag: function(r, R, FV) { with (Math) return -1.51 + 5*log10(r*R) + 0.016 * FV }
};
Ephemerides.Jupiter = {
    ...
    mag: function(r, R, FV) { with (Math) return -9.25 + 5*log10(r*R) + 0.014 * FV }
};

Ephemerides.Saturn = {
    N: function(d) { return 113.6634 + 2.38980E-5 * d },
    ...
    mag: function(r, R, FV, ring_magn) { with (Math) return -9.0  + 5*log10(r*R) + 0.044 * FV + ring_magn }
};

Ephemerides.Uranus = {
    N: function(d) { return  74.0005 + 1.3978E-5 * d },
    i: function(d) { return 0.7733 + 1.9E-8 * d },
    w: function(d) { return  96.6612 + 3.0565E-5 * d },
    a: function(d) { return 19.18171 - 1.55E-8 * d },
    a_units: "AU",
    e: function(d) { return 0.047318 + 7.45E-9 * d },
    M: function(d) { return 142.5905 + 0.011725806 * d },
    position: Ephemerides.generic_position,
    perturbation: function(eph, planet) {
        var Mj = eph.Jupiter.M;		// Mean anomaly of Jupiter
        var Ms = eph.Saturn.M;		// Mean anomaly of Saturn
    ...
    d: function(r) { return 65.8 / r }, /* polar 62 */
    mag: function(r, R, FV) { with (Math) return -7.15 + 5*log10(r*R) + 0.001 * FV }
};

Ephemerides.Neptune = {
    N: function(d) { return 131.7806 + 3.0173E-5 * d },
    i: function(d) { return 1.7700 - 2.55E-7 * d },
    w: function(d) { return 272.8461 - 6.027E-6 * d },
    a: function(d) { return 30.05826 + 3.313E-8 * d },
    a_units: "AU",
    e: function(d) { return 0.008606 + 2.15E-9 * d },
    M: function(d) { return 260.2471 + 0.005995147 * d },
    position: Ephemerides.generic_position,
    d: function(r) { return 62.2 / r }, /* polar 60.9 */
    mag: function(r, R, FV) { with (Math) return -6.90 + 5*log10(r*R) + 0.001 * FV }
};
